...
import { MemoizedQuestionsGridView } from "./QuestionsGrid";
...
function QuestionList({ match }) {
  ...
  useEffect(() => {
    ...
    getAllQuestionsByAccountId(user.accountId).then((data) => {
      setQuestions(data);
  ...
  const handleClassOnChange = (e) => {
    ...
    if (e.target.value === "") {
      ...
      const subjectsForTheClass = getSubjectsByClassID(
        classSubjects,
        e.target.value
      );
      setSubjects(subjectsForTheClass);

      const classQuestions = questionList.filter(
        (q) => q.classId === Number(e.target.value)
      );
  ...
  const handleSubjectOnChange = (e) => {
    ...
    const filteredQuestions = questionList.filter(
      (q) => q.classSubjectId === parseInt(e.target.value)
    );
  ...
  const showLoadingMessage = () => {
    ...
    return (
      <div
        ...
        style={{ display: values.isLoading ? "" : "none" }}
      >
        <Spinner />
      </div>
    );
  ...
  const showEmptyMessage = () => {
    ...
    return (
      <div
        ...
        style={{
          display:
            !values.isLoading && questions && questions.length === 0
              ? ""
              : "none",
        }}
      >
  ...
  //================================================
  const showQuestionsListView = () => {
    return (
      <div className="row m-0 p-0 w-100">
        <div className="col-12">
          <div className="bg-text-muted text-dark text-left py-2 mb-2">
            <b>Select Class & Subject:</b>
            <br />
            {populateClasses()}&nbsp;&nbsp;
            {populateSubjects()}
          </div>
                      ...
                      <ul className="optionList">
                        {q.options.map((o, i) => (
                          <li
                            key={o.id}
                            className={
                              o.isAnswer ? "list-answer" : "list-option"
                            }
                          >
  ...
  const showQuestionsGridView = () => {
    return (
      <div className="row m-0 p-0 w-100">
        <div className="col-12">
          <MemoizedQuestionsGridView questions={questions} />
        </div>
      </div>
    );
  };
  ...
  //================================================
  return (
    <LayoutTeacher title="Questions List">
      <div className="float-right d-none" style={{ marginTop: "-20px" }}>
        <Link to="#" onClick={() => setView("grid")}>
          Grid View
        </Link>
        &nbsp;&nbsp;::&nbsp;&nbsp;
        <Link to="#" onClick={() => setView("list")}>
          List View
        </Link>
      </div>
      {showLoadingMessage()}
      ...
      <div>{/* {JSON.stringify(questions, null, 4)} */}</div>
    </LayoutTeacher>
  );
}
